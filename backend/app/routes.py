...
def login():
    ...
    try:
        ...
        login_user(user)
        return jsonify({'success': True, 'msg': 'Inicio de sesión exitoso', 'user_id': user.id, 'username': user.username,'role': user.role}), 200
    except ValueError as e:
...
def import_chatbot_xml():
    ...
    try:
        ...
        if not xml_content.strip():
            return jsonify({
                'success': False, 'error': 'Archivo XML vacío.'}), 400
        ...
        return jsonify({'success': False, 'error': f"Error al parsear XML: {str(e)}"}), 400
